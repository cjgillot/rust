...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    pub(super) fn check_expr_with_expectation(
        ...
        let is_try_block_generated_unit_expr = match expr.kind {
            ExprKind::Call(_, ref args) if expr_span.is_desugaring(DesugaringKind::TryBlock) => {
                args.len() == 1
    ...
    fn check_named_place_expr(&self, oprnd: &'tcx hir::Expr<'tcx>) {
        ...
        if !is_named {
            struct_span_err!(
                self.tcx.sess,
                self.tcx.hir().span(oprnd.hir_id),
                E0745,
                "cannot take address of a temporary"
            )
            .span_label(self.tcx.hir().span(oprnd.hir_id), "temporary value")
            .emit();
        }
    }
    ...
    fn check_expr_return(
        ...
        if self.ret_coercion.is_none() {
            ...
            if self.ret_coercion_span.borrow().is_none() {
                *self.ret_coercion_span.borrow_mut() = Some(self.tcx.hir().span(e.hir_id));
            }
            self.check_return_expr(e);
        } else {
            let mut coercion = self.ret_coercion.as_ref().unwrap().borrow_mut();
            if self.ret_coercion_span.borrow().is_none() {
                *self.ret_coercion_span.borrow_mut() = Some(expr_span);
            }
    ...
    fn check_expr_assign(
        ...
        if expected_ty == self.tcx.types.bool {
            ...
            let left = self.tcx.sess.source_map().span_to_snippet(self.tcx.hir().span(lhs.hir_id));
            let right = self.tcx.sess.source_map().span_to_snippet(self.tcx.hir().span(rhs.hir_id));
            if let (Ok(left), Ok(right)) = (left, right) {
                let help = format!("{} == {}", left, right);
                err.span_suggestion(expr_span, msg, help, Applicability::MaybeIncorrect);
            } else {
                err.help(msg);
            }
            err.emit();
