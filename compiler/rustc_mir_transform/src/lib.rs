...
fn mir_const(tcx: TyCtxt<'_>, def: ty::WithOptConstParam<LocalDefId>) -> &Steal<Body<'_>> {
    ...
    if !tcx.sess.opts.unstable_opts.thir_unsafeck {
        if let Some(param_did) = def.const_param_did {
            tcx.execute_no_dependency().unsafety_check_result_for_const_arg((def.did, param_did));
        } else {
            tcx.execute_no_dependency().unsafety_check_result(def.did);
        }
    }

    // has_ffi_unwind_calls query uses the raw mir, so make sure it is run.
    tcx.execute_no_dependency().has_ffi_unwind_calls(def.did);
...
fn mir_drops_elaborated_and_const_checked(
    ...
    {
        tcx.execute_no_dependency().mir_generator_witnesses(def.did);
    }
...
fn inner_optimized_mir(tcx: TyCtxt<'_>, did: LocalDefId) -> Body<'_> {
    ...
    match tcx.hir().body_const_context(did) {
        ...
        Some(hir::ConstContext::ConstFn) => tcx.execute_no_dependency().mir_for_ctfe(did),
        None => {}
