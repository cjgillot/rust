...
use rustc_middle::ty::{self, ParamEnv, Ty, TyCtxt, TypeFlags, TypeFoldable, TypeFolder};
...
impl MirPass<'_> for DynErased {
    fn run_pass<'tcx>(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        rustc_middle::mir::dump_mir(tcx, None, "DynErased", &"Body", &erased_mir, |_, _| Ok(()));
        ...
        body.dyn_erased_body = Some(Box::new(erased_mir));
    }
}

fn dyn_erased(tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) -> Result<Body<'tcx>, String> {
    if body.arg_count == 0 {
        // Do not run for constants.
        return Err("rustc_dyn: constants are not allowed".to_owned());
    }
    if !body.is_polymorphic {
        // There is not point in running.
        return Err("rustc_dyn: function must be polymorphic".to_owned());
    }
    if let DefKind::Closure | DefKind::Generator = tcx.def_kind(body.source.instance.def_id()) {
        // Skip closures and generators.
        return Err("rustc_dyn: closures and generators are not handled".to_owned());
    }
    if body
        .has_type_flags(TypeFlags::HAS_KNOWN_CT_PARAM | TypeFlags::HAS_UNKNOWN_DEFAULT_CONST_SUBSTS)
    {
        // We do not handle this yet.
        return Err("rustc_dyn: const generics are not handled".to_owned());
    }

    let param_env = tcx.param_env_reveal_all_normalized(body.source.def_id());
    let types = gather_types(tcx, param_env, &*body)?;

    // We start modifying the body from this point on.
    let locals = gather_constants(tcx, param_env, body);
...
fn gather_types(
    ...
    while let Some(ty) = types.pop() {
        ...
        if let Err(err) = layout {
            return Err(format!("rustc_dyn: unknown layout for {:?}: {:?}", ty, err));
        }
        ...
        if let ty::FnDef(def_id, substs) = ty.kind() {
            ...
            if f.abi() == Abi::RustIntrinsic || f.abi() == Abi::PlatformIntrinsic {
                ...
                return Err(format!("rustc_dyn: unhandled intrinsic {:?}", ty));
            }
...
fn gather_constants(
    ...
    for data in bb.iter_mut() {
        if let Some(terminator) = &mut data.terminator {
            if let TerminatorKind::Drop { place, target, unwind } = terminator.kind {
                ...
                if !place_ty.potentially_has_param_types_or_consts() {
                    continue;
                }
                ...
                let drop_fn = Operand::Constant(Box::new(Constant {
    ...
    let mut calls = ConstantGatherer { tcx, param_env, locals };
    calls.visit_body(body);
    calls.locals
}
...
fn build_trampoline(
    ...
    for (decl, operand) in locals.decls.iter() {
        ...
        FnReifier { tcx, param_env }.visit_statement(&mut init_stmt, location);
...
impl<'tcx> MutVisitor<'tcx> for ConstantGatherer<'tcx> {
    ...
    fn visit_operand(&mut self, operand: &mut Operand<'tcx>, location: Location) {
        self.super_operand(operand, location);
        if let Operand::Constant(ref constant) = &*operand {
            if constant.potentially_has_param_types_or_consts() {
                let source_info = SourceInfo::outermost(constant.span);
                let mut const_ty = constant.literal.ty();
                if let ty::FnDef(def_id, substs) = const_ty.kind() {
                    const_ty = mk_fn_ptr(self.tcx, self.param_env, *def_id, substs);
                }
...
impl TypeFolder<'tcx> for TypeEraser<'tcx> {
    ...
    fn fold_ty(&mut self, ty: Ty<'tcx>) -> Ty<'tcx> {
        ...
        assert!(!ty.potentially_has_param_types_or_consts());
        ty
    }
